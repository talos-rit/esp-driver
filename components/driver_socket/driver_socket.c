...
void driver_socket_task(void *arg) {
  ...
  // address_info
  int res = getaddrinfo(&task_args->config.ip[0], &task_args->config.port[0],
                        &hints, &address_info);
  if (res != 0 || address_info == NULL) {
    ESP_LOGE(TAG,
             "couldn't get hostname for `%s` "
             "getaddrinfo() returns %d, addrinfo=%p",
             &task_args->config.ip[0], res, address_info);
    goto error;
  }
  ...
  ESP_LOGI(TAG, "Socket bound on %s:%s", &task_args->config.ip[0],
           &task_args->config.port[0]);
...
esp_err_t driver_socket_init(driver_socket_handle_t *socket_handle,
  ...
  if (server_ready == NULL) {
    ESP_LOGE(TAG, "Unable to create server_ready semaphore");
    free(task_args);
    return ESP_ERR_NO_MEM;
  }

  task_args->socket = *socket_handle;
  task_args->config = *config;
  task_args->server_ready = server_ready;

  xTaskCreate(driver_socket_task, "driver_socket_task", 4096, task_args, 5,
              NULL);
