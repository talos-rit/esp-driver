...
#define ENCODER_LOW_LIMIT \
  INT16_MIN  // These are not configurable due to overflow functionality!
#define ENCODER_HIGH_LIMIT \
  INT16_MAX  // These are not configurable due to overflow functionality!
...
typedef struct {
  ...
  int glitch_filter_ns; /**< Pulses shorter than this will not inc/dec the count
                         */
  int resolution;   /**< Number of slots in the encoder wheel, used to calculate
                       the real world angle */
  float gear_ratio; /**< For every X number of rotations of the encoder wheel,
                       the limb it controls moves once */
 ...
 *
 * This function initializes the Pulse Counter hardware module for a specific
 * encoder.
 *
 ...
 *
 * Starts the Pulse Counter module for the specified encoder. The count is set
 * to 0.
 *
 * @param[out] handle Pointer to encoder handle structure
 *
 ...
 *
 * Retrieves the raw pulse count of the encoder. This count is signed, for every
 * slot in the encoder wheel this count will increment or decrement (based on
 * direction) by 4.
 *
 * @param[out] handle Pointer to encoder handle structure
 ...
 *
 * Resets pulse count of the encoder to 0. This function should only be called
 * when an end stop has triggered. If called otherwise, all absolute angles will
 * be incorrect.
 *
 * @param[out] handle Pointer to encoder handle structure
 *
 ...
 * current pulse count, default pulse count (0), and wheel resolution. This
 * angle is signed, and can go outside 360.
 *
 * @param[out] handle Pointer to encoder handle structure
 * @param[in] config Pointer to configuration structure
 ...
 *
 * Retrieves the angle of the encoder wheel. This angle is calculated using the
 * current pulse count, default pulse count (0), wheel resolution, limb gear
 * ratio, and default limb angle. This angle is signed, and can go outside
 * 360(though it may not depending on the robot anatomy).
 *
 * @param[out] handle Pointer to encoder handle structure
 * @param[in] config Pointer to configuration structure
 * @param[out] wheel_angle Pointer to angle value desired
