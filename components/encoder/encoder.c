...
esp_err_t encoder_init(encoder_handle_t* handle, encoder_config_t* config) {
  ...
  pcnt_unit_config_t unit_config = {
      ...
      .flags.accum_count = 1  // accumulate the count in the event the count
                              // goes past limits (count is reset to 0)
  };
...
esp_err_t encoder_get_wheel_angle(encoder_handle_t* handle,
  ...
  *wheel_angle = ((float)pulse_count * 360.0f) /
                 resolution;  // Due to 360 degrees in 1 rotation.
