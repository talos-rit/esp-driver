...
#define POLAR_PAN_SPEED PCA9685_PWM_MAX * atof(CONFIG_DRIVER_MOTORHAT_PAN_SPEED)
...
static motorhat_direction_t delta_to_direction(int8_t delta) {
    ...
    return MOTORHAT_DIRECTION_BRAKE;
}
...
esp_err_t motorhat_home(uint16_t delay_ms) {
    ...
    // TODO: Implement homing logic using motorhat_set_motor_direction, motorhat_set_motor_speed, and limit switch state from signal bus
    return ESP_OK;
}
...
esp_err_t motorhat_polar_pan(int16_t delta_azimuth, int16_t delta_altitude,
    ...
    // TODO: Implement polar pan logic using motorhat_set_motor_direction and motorhat_set_motor_speed
    return ESP_OK;
}
...
esp_err_t motorhat_polar_pan_start(int8_t delta_azimuth, int8_t delta_altitude) {
  ...
  esp_err_t err = motorhat_set_motor_speed(s_handle, axis_motor[MOTORHAT_AXIS_AZIMUTH], 0);
  if (err != ESP_OK) {
    return err;
  }
  err = motorhat_set_motor_speed(s_handle, axis_motor[MOTORHAT_AXIS_ALTITUDE], 0);
  if (err != ESP_OK) {
    return err;
  }

  // Set directions based on deltas
  err = motorhat_set_motor_direction(s_handle, axis_motor[MOTORHAT_AXIS_AZIMUTH], delta_to_direction(delta_azimuth));
  if (err != ESP_OK) {
    return err;
  }
  err = motorhat_set_motor_direction(s_handle, axis_motor[MOTORHAT_AXIS_ALTITUDE], delta_to_direction(delta_altitude));
  if (err != ESP_OK) {
    return err;
  }

  // Set speeds to a default value
  err = motorhat_set_motor_speed(s_handle, axis_motor[MOTORHAT_AXIS_AZIMUTH], POLAR_PAN_SPEED);
  if (err != ESP_OK) {
    return err;
  }
  return motorhat_set_motor_speed(s_handle, axis_motor[MOTORHAT_AXIS_ALTITUDE], POLAR_PAN_SPEED);
}
